...
public interface Solicitudes_Repository extends JpaRepository<Solicitudes, Long> {
  ...
  @Query("SELECT new com.proyectosgrt.demo.DTO.EstTecnicoDTO (COUNT(s.idsol), s.nodoctecnico, p.pnom, p.pape) FROM Solicitudes s INNER JOIN Personas p ON s.nodoctecnico = p.nodoc WHERE s.idest = 3 GROUP BY s.nodoctecnico")
